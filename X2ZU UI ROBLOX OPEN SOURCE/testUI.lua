...
function Items:AddParagraph(ParagraphConfig)
    ...
    -- Update after a frame to ensure AbsoluteSize is known
    task.defer(UpdateSize)

    -- Also update if the paragraph's size changes (e.g., parent resized)
    Paragraph:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateSize)

    function ParagraphFunc:SetContent(content)
        content = content or "Content"
        ParagraphConfig.Content = content
...
-- ==================== FITUR TAMBAHAN DARI ALERT.LUA ====================
-- Fungsi ini bisa dipanggil setelah window dibuat: Alert(Window, Tabs)
-- Contoh: Alert(Window, Tabs) -- asumsikan Tabs berisi tab Info dan Misc
function Alert(Window, Tabs)
    local Players = game:GetService("Players")
    ...
    -- ==================== INFO SECTION ====================
    local InfoSection = Tabs.Info:AddSection({ Title = "Nemesis Announcements", Open = true })
...
<b>Use responsibly and enjoy the features!</b>
    ...
    InfoSection:AddParagraph({
        ...
        ButtonCallback = function()
            if setclipboard then
                ...
                Notify("Succesfully copied link!")  -- Gunakan Notify dari Velaris
            end
        end
    })

    -- ==================== SERVER JOB ID SECTION ====================
    ...
    -- Freecam (diambil dari alert.lua, sudah diadaptasi)
